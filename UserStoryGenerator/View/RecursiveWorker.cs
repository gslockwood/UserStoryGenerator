        ...
        private void CreateNodesInternal(IList<Model.IssueData.Issue> issues, TreeNode node, bool allIssue = false)
        {
            foreach( Model.IssueData.Issue issue in issues )
            {
                ...
                if( issue.IssueType.Equals(JiraIssueType.Story) )
                    ...
                    toolTipText = $"Issue Type={issue.IssueType} OriginalEstimate={IssueUtilities.FormatEstimateTime(issue.OriginalEstimate.Value)}";
                ...
                if( issue is IssueData.TaskIssue )
                {
                    issueNode = new TreeNodeExTask((IssueData.TaskIssue)issue)
                    {
                        ToolTipText = toolTipText,
                        // mark the issue as generated by userStory generation  (false) or 'all issue' run for a user story (true)
                        Tag = allIssue,
                ...
                if( issue is IssueData.TestTask )
                {
                    issueNode = new TreeNodeExTest((IssueData.TestTask)issue)
                    {
                        ToolTipText = toolTipText,
                        // mark the issue as generated by userStory generation  (false) or 'all issue' run for a user story (true)
                        Tag = allIssue,
