...
namespace UserStoryGenerator.Model
{
    public class Model
    {
        ...
        public int UserStoryMaxOutputTokens { get; internal set; } = 2;
        public int AllIssueMaxOutputTokens { get; internal set; } = 2;
        ...
        internal async Task ProduceUserStories(string jiraProject, string productName, string target, bool addQATests, bool addSubTasks, bool addDescriptions, int maxStories)
        {
            ...
            Logger.Info(Settings.GeminiModel);

            IssueGeneratorUserStories? issueGenerator = new(issueGeneratorBaseInputArgs, this.UserStoryMaxOutputTokens);
        ...
        internal async Task ProcessStoryList(string productName, bool addQATests, bool addSubTasks, bool addDescriptions, int maxStories, List<StoryPackage> list)//async Task 
        {
            ...
            foreach( StoryPackage storyPackage in list )
            {
                ...
                IssueGeneratorAllIssues? issueGenerator = new(issueGeneratorBaseInputArgs, AllIssueMaxOutputTokens);
                issueGenerator.Completed += (args) =>
                {
        ...
        private void ProcessResults(IssueGeneratorBaseArgs arg, Result result)
        {
            ...
            {
                Logger.Info(result.Answer.Length);
                Logger.Info(this.UserStoryMaxOutputTokens);
                result.ErrorCode = -99;
            }
            //
        }
